...
(define-public (is-dao-or-extension)
	(ok (asserts! (or (is-eq tx-sender .chain-dao) (contract-call? .chain-dao is-extension contract-caller)) err-unauthorised))
)
...
(define-public (add-proposal (proposal <proposal-trait>) (data {start-block-height: uint, end-block-height: uint, proposer: principal}))
	(begin
		(try! (is-dao-or-extension))
		(asserts! (is-none (contract-call? .chain-dao executed-at proposal)) err-proposal-already-executed)
...
(define-private (is-governance-token (governance-token <chain-usda-trait>))
...
(define-public (vote (amount uint) (for bool) (proposal principal) (governance-token <chain-usda-trait>))
	(let
		(
		...
		(contract-call? governance-token cw-lock amount tx-sender)
	)
)
...
(define-public (conclude (proposal <proposal-trait>))
	(let
		...
		(and passed (try! (contract-call? .chain-dao execute proposal tx-sender)))
		(ok passed)
	)
)
...
(define-public (reclaim-votes (proposal <proposal-trait>) (governance-token <chain-usda-trait>))
	(let
		(
		...
		(contract-call? governance-token cw-unlock votes tx-sender)
	)
)

(define-public (reclaim-and-vote (amount uint) (for bool) (proposal principal) (reclaim-from <proposal-trait>) (governance-token <chain-usda-trait>))
	(begin
