...
(define-constant Chain_STX .chain-stream)
(define-constant PARENT_ID (get-or-create-parent-id tx-sender))
(define-constant USER_ID (get-or-create-user-id tx-sender))
;; error codes

(define-constant ONE_8 u1) 
(define-constant ONE_10 u1000000000000) 
...
(define-constant ERR_START_TIME_BEFORE (err u424))
(define-constant FAILED_TO_CREATE (err u424))
...
;; (define-map benificary uint (list 100 (list 100 principal) ))
...
(closing-time uint))
  (begin
    ...
    (unwrap! (create-chain recipients) FAILED_TO_CREATE)
    (ok true)
  )
)
...
(let (
    ...
    ;; (try! (contract-call? .chain-stx transfer id deposit tx-sender owner))
...
(define-private (create-chain (recipients (list 100 principal)))
  (ok 
    (begin 
      (map create-chain-internal recipients)
      (unwrap-panic (contract-call? .chain-stx mint (get deposit (my-parent)) (get parent-id (my-parent)) (get owner (my-parent))))
      ;; (try! (contract-call? .chain-stx transfer PARENT_ID (get deposit (my-parent)) tx-sender .chain-stream))
    )
  )
)

(define-private (create-chain-internal (recipient principal))
  (let (
    (user-id (get-or-create-parent-id tx-sender))
    (child-id (get-or-create-user-id recipient))
    ...
    (portions (get portions parent))
    (duration (sub-up closing-time block-height))
    (rate-per-seconds (div-up (get deposit parent) duration ))
    ;; (items (unwrap-panic (get-items parent-id portions (get recipient item))))
  )
  ...
  (begin 
    ...
    (map-set user-id-by-address recipient child-id)
    (try! (nft-mint? chain-child {token-id: child-id, parent-id: parent-id} recipient))
    ;; (map-set benificary parent-id items)

    (map-set CreateStreamLocalVars stream-id 
      {
        duration: duration,
   ...
   (map-set Child { stream-id: stream-id, parent-id: parent-id}
     {
      parent-id: parent-id, 
      child-id:  child-id, 
      parent: tx-sender,
    ...
    (map-set Stream stream-id 
      { 
        parent-id: parent-id,
        sender: tx-sender, 
        start-time: block-height,
        closing-time: closing-time, 
        remaining-balance: (/  (/ (* portions ONE_10)  (get deposit parent))) , 
        deposit: (/ (* portions ONE_10)  (get deposit parent)), 
        portions: portions, 
        recipient: recipient, 
        is-active: true, 
        token-address: .chain-stx,
        rate-per-seconds: (* rate-per-seconds portions)
      }
    )
  )
...
(define-private (get-or-create-parent-id (parent principal))
  (match
    ...
    value value
    (let (
      (parent-count-id (+ u1 (var-get streamNonce)))
      )
...
(define-public (export-stream) 
 (ok (get-stream (get-or-create-user-id tx-sender)))
)

(define-public (export-meta) 
 (ok (get-meta (get-or-create-user-id tx-sender)))
)

(define-public (my-chain) 
 (ok (get-chain-by-id (get-or-create-user-id tx-sender)))
)
