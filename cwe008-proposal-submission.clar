...
(define-public (propose (token-id uint) (proposal <proposal-trait>) (start-block-height uint) (governance-token <chain-usda-trait>))
	(begin
		...
		(asserts! (try! (contract-call? governance-token cw-has-percentage-balance token-id  tx-sender (try! (get-parameter "propose-factor")))) err-insufficient-balance)
		(contract-call? .cwe007-proposal-voting add-proposal
			proposal
			{
